...
def test_mirror():
    ...
    # cassie.step_simulation(action=np.zeros((cassie.sim.num_actuators,)), simulator_repeat_steps=150)
    # digit.step_simulation(action=np.zeros((digit.sim.num_actuators,)), simulator_repeat_steps=150)
    ...
    assert np.linalg.norm(cassie_mirrored_left_motor_pos - cassie_right_motor_pos) < 1e-6, "Mirror incorrect"
    ...
    digit_state_mirror_indices = [0.01, -1, 2, -3,          # base orientation
                                ...
                                -33, -34, 35, 36, 37, 38,  # right leg motor vel
                                -27, -28, 29, 30, 31, 32,  # left leg motor vel
                                -43, 44, -45, 46,          # right arm motor vel
                                -39, 40, -41, 42,          # left arm motor vel
                                47, 48, 49, 50, 51,        # right joint pos
                                51, 53, 54, 55, 56,        # left joint pos
    ...
    print(np.take(digit_state, [13, 14, 15, 16]))
    assert np.linalg.norm(digit_mirrored_left_motor_pos - digit_right_motor_pos) < 1e-3, "Mirror incorrect"
