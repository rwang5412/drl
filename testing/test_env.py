...
def test_all_env():
    bot_list = ["cassie", "digit"]
    child_env_list = [[LocomotionClockEnv, "mujoco"]]
    ...
    for pair in child_env_list:
        for bot in bot_list:
            try:
                test_child_env_step(test_env=pair[0], test_sim=pair[1], bot=bot)
    ...
    for pair in child_env_list:
        for rew_pair in reward_list:
            for bot in bot_list:
                try:
                    test_child_env_reward(pair[0], pair[1], rew_pair[0], rew_pair[1], bot=bot)
    ...
    for pair in child_env_list:
        for rew_pair in reward_list:
            for bot in bot_list:
                try:
                    test_env_factory(pair[0], pair[1], rew_pair[0], rew_pair[1], bot=bot)
                    print(f"{OKGREEN}Pass test with {pair[0].__name__} and {pair[1]}, clock " \
                        f"{rew_pair[0]}, and reward {rew_pair[1]}.{ENDC}")
                except Exception:
                    print(f"{FAIL}{pair[0].__name__} with {pair[1]}, clock {rew_pair[0]}, and reward " \
...
def test_base_env_step(test_env, test_sim, bot):
...
def test_child_env_step(test_env, test_sim, bot):
...
def test_child_env_reward(test_env, test_sim, clock_type, reward, bot):
...
def test_env_factory(test_env, test_sim, clock_type, reward, bot):
