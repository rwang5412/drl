...
## Changelog
- 04/19/24, PR#106: `udp.py` for running Cassie policies on hardware renamed to `cassie_udp.py`. Also the bug fixed in this PR will need to be applied to your own custom envs if you have them. Just a single line change in the `check_observation_action_size` if check to account for Cassie hardware case. See PR file changes for details.
- 04/17/24, PR#107: Breaking changes made to `plot_log.py`. Old log files will no longer be compatible and can not be loaded or plotted.
