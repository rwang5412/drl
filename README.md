...
Currently only policy inputs, outputs, time, and `orient_add` are logged. For now if you want to add logging of your own values, you have to define a global variable in the main function, initialize it with `log_size` entries (basically want to allocate memory of the list only once), add it to the `global` list in `save_log` and `run`, write your data to the log variable in `run`, and add it to the `data` dictionary that gets saved in `save_log`. An easier way to add custom logs may come later.
